...
from flightsim.numpy_json_encoder import NumpyJSONEncoder
...
class World(object):
    ...
    def draw_line(self, ax, points, color=None):
        ...
        points = [np.interp(t, ts, points[:,k]) for k in range(3)]
        ax.scatter(points[0], points[1], points[2], s=3, c=color, edgecolors='none', depthshade=False)
